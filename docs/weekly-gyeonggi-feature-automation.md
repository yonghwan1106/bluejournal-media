...
이 자동화는 매주 토요일 오전에 경기도 공식자료를 수집해 주제를 선정하고 심층기사를 작성한 뒤, 독립 RSI 검수와 대표 이미지 공개 검증을 모두 통과한 기사만 `scheduled` 상태로 저장한다. 실제 공개는 기존 `publish-scheduled` 작업이 담당한다.

| 단계 | 한국 시각(KST) | Vercel Cron(UTC) | 경로 |
| --- | --- | --- | --- |
| 준비 실행 | 토요일 07:00 | 금요일 `0 22 * * 5` | `/api/cron/weekly-gyeonggi-feature/prepare` |
| 재시도 | 토요일 08:00 | 금요일 `0 23 * * 5` | `/api/cron/weekly-gyeonggi-feature/retry` |
| 공개 | 토요일 09:00 예약 | 기존 매시 정각 작업 | `/api/cron/publish-scheduled` |

기사의 `published_at`은 해당 토요일 `09:00:00+09:00`로 고정된다. Vercel Cron 전달이 중복되거나 준비·재시도 실행이 겹쳐도 주차별 실행 키와 기사 자동화 키가 중복 생성을 막는다.
...
3. AI Gateway 구조화 출력으로 주제와 서로 다른 공식자료 URL을 최소 3개 선택한다.
4. 선택된 상세 원문을 다시 가져와 정확한 `BS_CODE`와 문서 번호, 제목, 날짜, 본문을 확인한다.
5. 기사를 정확히 `현황 → 원인 → 데이터·사례 → 반론 → 대안·전망`의 5단으로 작성한다. 제목·소제목을 제외한 리드, 본문, 결론은 공백 포함 2,500~3,500자를 목표로 하며 2,500~5,000자만 허용한다.
6. 작성 호출과 별도의 AI Gateway 구조화 호출로 RSI 검수를 수행한다. 결과는 `RSI_PASS`, `REVISE`, `HOLD` 중 하나다. `REVISE`는 최대 두 번까지 전체 수정하고, 매번 이전 판정 대화와 분리된 새로운 독립 호출로 재검수한다.
7. 최종 `RSI_PASS`이고 로컬 구조 검사까지 통과한 경우에만 대표 이미지를 만든다. AI 이미지 생성 또는 업로드·공개 검증이 실패하면 자체 제작 SVG를 R2에 올린다.
8. R2 공개 URL을 `HEAD`로 확인하고, 헤더가 불충분하거나 `HEAD`가 허용되지 않으면 Range `GET`으로 다시 확인한다. HTTP 성공, `image/*`, 최소 800바이트를 모두 충족하지 않으면 기사를 저장하지 않는다.
9. 통과 기사를 `section=특집`, `region=경기`, `display_slot=헤드라인`, `reporter_name=경인블루저널 박용환`, `status=scheduled`로 저장한다. 본문에는 섹션별 공식 근거 링크와 최종 출처 목록을 넣는다. AI 이미지는 캡션에 `AI 생성 이미지`를, SVG 폴백은 자체 제작 자료 이미지임을 명시한다.

`RSI_PASS`가 아니거나 공식자료가 3건 미만이거나 분량·구조·이미지 검증이 실패하면 새 기사 행은 만들어지지 않는다.
...
- `WEEKLY_FEATURE_RSI_MODEL`: 선택, 기본 `openai/gpt-5.4-nano`
- `WEEKLY_FEATURE_IMAGE_MODEL`: 선택, 기본 `openai/gpt-image-2`

모델 값은 반드시 `provider/model` 문자열이어야 한다. 로컬에서 Vercel OIDC를 사용할 때는 최신 프로젝트 환경을 다시 받아 토큰을 갱신한다. OIDC를 쓰지 않는 로컬 검증은 별도의 AI Gateway API 키 설정이 필요할 수 있다.
...
`weekly_feature_runs`는 멱등 상태의 기준이고, `cron_runs`는 각 호출의 운영 이력이다. 08:00 재시도 후에도 `scheduled`가 아니면 `error_text`, Vercel 함수 로그, AI Gateway 사용 기록, R2 공개 URL 순서로 확인한다. 09:00가 지나면 자동화는 새 기사를 만들지 않으며, 편집자가 근거와 이미지를 직접 확인해 수동 발행 여부를 결정해야 한다.
